...
const getSecret = ({ decrypt, persistence }) => {
  ...
  return new Promise((resolve, reject) => {
    let failed = false;
    let encrypted = '';
    const readStream = persistence.read('secret');
    ...
    readStream.on('end', () => {
      ...
      return resolve(decrypt(encrypted));
    });
...
const setSecret = ({ encrypt, persistence }, secret) => {
  ...
  // NOTE: not adding events for secrets at this time
  return encrypt(secret).then(
    encrypted =>
      new Promise((resolve, reject) => {
        let failed = false;
        const writeStream = persistence.write('secret');
        writeStream.on('end', () => {
          if (failed) {
            return;
          }
...
const getPackageStorage = (context, packageInfo) => {
  ...
  */
  const updatePackage = (
    ...
    const done = (err, emit) =>
      persistence.unlock(pkgPath).then(() => {
        setImmediate(() => {
          emit();
          onEnd(err);
        });
