...
const getPackageStorage = (context, packageInfo) => {
  ...
  const getStoragePath = (itemPath = '') => {
    return itemPath? packageInfo : path.join(packageInfo, itemPath);
  }
  ...
  */
  ...
  const readPackage = (pkgName, done) => {
    ...
    events.emit('read-package', { correlationId: id, filename: pkgPath, pkgName });
    events.emit('read-package:start', { correlationId: id, filename: pkgPath, pkgName });

    return readPkgPathData({ pkgPath })
    .then(
      (pkgInfo) => setImmediate(() => {
        events.emit('read-package:complete', { correlationId: id, filename: pkgPath, pkgName, pkgInfo });
        return done(null, pkgInfo);
      }),
      (e) => setImmediate(() => {
        events.emit('read-package:error', { correlationId: id, filename: pkgPath, pkgName, error: e.message });
        return done(e);
      })
    );
  };
  ...
  */
  const writePackage = ({
    ...
    events.emit(`${action}-package`, { correlationId: id, filename: pkgPath, pkgName, pkgInfo });
    events.emit(`${action}-package:start`, { correlationId: id, filename: pkgPath, pkgName, pkgInfo });
    ...
    writeStream.on('error', (e) => {
      events.emit(`${action}-package:error`, { correlationId: id, filename: pkgPath, pkgName, error: e.message });
      writeStream.end();
      return done(err);
    });
    ...
    writeStream.end(pkgData, () => {
      events.emit(`${action}-package:complete`, { correlationId: id, filename: pkgPath, pkgName, pkgData });
      return done();
    });
  };
  ...
  */
  const updatePackage = (
    ...
    const done = (err, emit) => persistence.unlock(pkgPath)
      .then(() => {
        setImmediate(() => {
          emit();
          onEnd(err);
    ...
    events.emit('update-package', { correlationId: id, filename: pkgPath, pkgName });
    events.emit('update-package:start', { correlationId: id, filename: pkgPath, pkgName });

    return persistence.lock(pkgPath)
    .then(() => readPkgPathData({ pkgPath }))
    .then((rawPkgInfo) => promisify(updateHandler)(rawPkgInfo))
    .then((updatedPkgInfo) => promisify(onWrite)(pkgName, transformPackage(updatedPkgInfo)))
    .then(
      // NOTE: this odd looking construct ensures we finish unlocking our pkgPath
      // before we emit our event and then call our onEnd callback
      () => done(null, () => {
        events.emit('update-package:complete', { correlationId: id, filename: pkgPath, pkgName });
      }),
      (e) => done(e, () => {
        events.emit('update-package:error', { correlationId: id, filename: pkgPath, pkgName, error: e.message });
      })
    );
  };
  ...
  */
  const deletePackage = (target, done) => {
    ...
    const id = uuid.v4();
    // TODO: figure out a reasonable pkgPath
    const targetPath = target !== packageInfo? target : getStoragePath(packageInfoFile); // <- use our pkgPath function

    events.emit('delete-package', { correlationId: id, filename: targetPath, target });
    events.emit('delete-package:start', { correlationId: id, filename: targetPath, target });

    return persistence.remove(targetPath)
    .then(
      () => setImmediate(() => {
        events.emit('delete-package:complete', { correlationId: id, filename: targetPath, target });
        return done();
      }),
      (e) => setImmediate(() => {
        events.emit('delete-package:error', { correlationId: id, filename: targetPath, target, error: e.message });
        return done(e);
      })
    );
  };
  ...
  const removePackage = (done) => {
    ...
    events.emit('remove-package:start', { correlationId: id, name: packageInfo });

    return persistence.remove(getStoragePath())
    .then(
      () => setImmediate(() => {
        events.emit('remove-package:complete', { correlationId: id, name: packageInfo });
        return done();
      }),
      (e) => setImmediate(() => {
        events.emit('remove-package:error', { correlationId: id, name: packageInfo, error: e.message });
        return done(e);
      })
    );
  };
