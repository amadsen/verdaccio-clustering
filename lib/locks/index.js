const init = ({ events, persistence }) => {
  ...
  /*
  TODO: add event listener for remote lock and unlock requests so they can be added to
  ...
  */
  ...
  function handleTargets(action, { targets, id }, done) {
      ...
      targets.map(
        ({ target, scope }) =>
          new Promise((resolve, reject) => {
            lockTargetHandlers[scope][action]({ target, id }, err => {
              if (err) {
                return reject(err);
              }
              return resolve();
  ...
  function prepareLocks({ targets, id }, done) {
    ...
    handleTargets('lock', { targets, id }, done);
  }

  function removeLocks({ id }, done) {
    console.log('Removing locks for', id);
    const targets = targetsForId[id];

    handleTargets('unlock', { targets, id }, done);
  }
