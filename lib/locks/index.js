...
const init = ({ events, persistence, sign, verify }) => {
  ...
  */
  ...
  /*
  The sync() function downloads newly updated packages from
  ...
  */
  const sync = ({ target, id, token }) => {
    return verify(token).then(decoded => {
      ...
      return hashTarget(target).then(sha256 => {
        /*
        ...
        */
        ...
        return new Promise((reject, resolve) => {
          events.on(`remote:download:${target}:${id}:${decoded.sha256}`, ds => {
            // ds should be stream which the events subsystem can populate any way
            // it sees fit, so long as the data is correct for the target, id, and decoded.sha
            if (
              lockTargets
                .forId(id)
                .filter(t => t.target === target && t.running)[0]
            ) {
              // we are still running this target - this should ALWAYS be true
              const ws = ds.pipe(
                persistence.write(target, { sha256: decoded.sha256 })
              );
              ws.on('error', e => reject(e));
              ws.on('end', () => resolve());
            }
            reject(new Error('Attempt to sync target without current lock'));
          });
          sign({
            target,
            id,
            ...
            events.emit(`remote:download`, downloadToken)
          );
        });
      });
    });
