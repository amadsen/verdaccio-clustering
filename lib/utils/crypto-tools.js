...
/*
TODO: extract encrypt and decrypt in to an external, testable module.
*/

const sign = (cluster, payload) =>
...
const encrypt = cluster => {
  ...
  const s = through2(
    function(chunk, enc, done) {
      ...
      if (!tempfile) {
        tempfile = fs.createWriteStream(path.join(__dirname, uuid.v4()));
      ...
      if (!decipherstream) {
        ...
        encryptedBuffer =
          totalLength === chunk.length
            ? chunk
            : Buffer.concat([encryptedBuffer, chunk], totalLength);
        ...
        if (encryptedBuffer.length >= 3) {
          ...
          if (
            ...
            ivlen > 0 &&
            taglen > 0 &&
            encryptedBuffer.length >= prefixLen
          ) {
            const keystart = 3;
